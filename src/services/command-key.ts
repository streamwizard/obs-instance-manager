...
// in memory, refresh it periodically, and re-fetch (rate-limited) on a mismatch
// so a DB rotation propagates without a restart.
...
export function startCommandKeyRefresh(): void {
  setInterval(() => void loadCommandKeyHash(), REFRESH_INTERVAL_MS);
}
